...
while ($linha = $consulta->fetch(PDO::FETCH_ASSOC)) {
    ...
    if (($intervalo->y == 0) && ($intervalo->m == 0) && ($intervalo->d == 0) && ($intervalo->h == 0) && ($intervalo->i >= 15) && ($intervalo->i < 60)) {
      ...
      echo $tabela. ' está dentro do radar do cron !. Ultima msg: '.$linha['ultimamsg'].'<br/>';
      ...
      if($flagheroi > 0){
        ...
        echo  $nomeheroi.' receberá um email de resgate referente ao chat com: '.$nomeanjo.'<br/>';
...
';
        ...
        //agora que o email foi enviado! vou gravar essa auditoria
        #auditoria
        try{
           $stmte2 = $conexao->prepare("INSERT INTO auditoriaresgate (idheroi,destino,assunto,dataenvio) VALUES (?,?,?,?)");
           $stmte2->bindParam(1, $idanjo , PDO::PARAM_STR);
           $stmte2->bindParam(2, $email , PDO::PARAM_STR);
           $stmte2->bindParam(3, $assunto , PDO::PARAM_STR);
           $stmte2->bindParam(4, $datacron , PDO::PARAM_STR);
           $executa2 = $stmte2->execute();

           if(!$executa2){
             echo "erro ao salvar na auditoria <br/>";
           }
        }
         catch(PDOException $e){
            echo $e->getMessage();
