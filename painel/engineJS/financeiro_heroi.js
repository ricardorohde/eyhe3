...
Query(document).ready(function(){
   jQuery('#form-pix').submit(function(){
      jQuery.ajax({
           type: "POST",
           url: "painel/enginePIX/emitirPIX.php",
           ...
           {
              alert(data);
             /*if(data == 'Falha ao gerar PIX, tente novamente.'){
               Swal.fire({
                 icon: 'error',
                 title: 'Oops...',
                 text: 'Falha ao emitir PIX! Preencha o campo CPF corretamente',
               })
             }else{
               //data == link de acesso ao boletão!
               Swal.fire({
                  title: '<strong> PIX GERADO! </strong>',
                  icon: 'success',
                  html:
                    '<a target="_blank" href="'+data+'">Clique aqui e acesse seu boleto</a> ',
                  showCloseButton: false,
